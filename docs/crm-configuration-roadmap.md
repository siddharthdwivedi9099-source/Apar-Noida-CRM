...
**Continue Phase A after the Lead + Account + Opportunity slices** by expanding the same custom-field persistence pattern to the next high-value CRM entities and then consuming the same field-definition contract in rendering:

1. Replicate the additive `custom_fields JSONB` pattern to the next CRM entities that already have custom-field metadata demand, starting with Contacts and then the next commercially active modules where the business case is strongest.
