...
1. Replicate the additive `custom_fields JSONB` pattern to the next CRM entities that already have custom-field metadata demand — Contacts is now done, so continue with the next commercially active modules (e.g. partners/resellers, support tickets) where the business case is strongest.
