...
1. Replicate the additive `custom_fields JSONB` pattern to the next CRM entities that already have custom-field metadata demand — Contacts and Support tickets are now done (the latter on the shared `apps/api/src/common/custom-fields.ts` engine), so continue with the remaining commercially active modules (e.g. partners/resellers, opportunities-adjacent renewal/expansion) where the business case is strongest, reusing the shared engine.
