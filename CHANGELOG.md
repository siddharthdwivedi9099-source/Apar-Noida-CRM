...
- Role-based demo logins + seeder (`scripts/seed-demo-users.mjs`, `npm run db:seed:demo-users`): one active demo user per canonical role (28 total), each assigned its role so logins demonstrate RBAC (verified: Super Admin ≈299 permissions vs Support Executive ≈22 vs Customer Portal User ≈4). Shared password `Demo@1234`; the Customer Portal User is linked to an active portal profile. Documented in `docs/user-guides/DEMO_LOGINS.md`. Idempotent.
...
- AI governance review (2026-06-24): verified all 17 governance areas in code (gateway, prompt/agent registries, RAG grounding, citations, escalation, low-confidence handling, human review, feedback, auditability, knowledge gaps). Added a prompt-variable sanitizer in `ai-gateway.service.ts` (`resolvePrompt`) that strips control characters, neutralizes `{{ }}` template delimiters, and clamps length on caller-supplied values — defense-in-depth for prompt-injection/abuse at the single chokepoint for every generative path. Created `docs/ai/AI_GOVERNANCE_REVIEW_REPORT.md`; updated `AI_GOVERNANCE.md`, `CUSTOMER_QUERY_AI_DESIGN.md`, `RAG_ARCHITECTURE.md`, and `AI_ASSISTANT_USER_GUIDE.md`.
